...
const CalendarPage = observer((props) => {
	...
	useEffect(() => {
		...
		fetch(
			"http://mediabit.ro/booking-staging/wp-json/wp/v2/users?per_page=500",
			requestOptions
		)
		...
		fetch(
			"http://mediabit.ro/booking-staging/wp-json/wp/v2/categories?acf_format=standard&per_page=100&orderby=slug",
			requestOptions
		)
			.then((response) => response.json())
			.then((result) => {
	...
	useEffect(() => {
		...
		if (users.length)
			fetch(
				`http://mediabit.ro/booking-staging/wp-json/wp/v2/posts/?data_start=${
	...
	useEffect(() => {
		if (!props.adminId || (props.adminId && props.adminId !== props.id)) {
			...
			const weekStart = formatDateYMD(getStartWeek(view));
			fetch(
				`http://mediabit.ro/booking-staging/wp-json/times/ocupied/?data_start=${
	...
	const addEventHandler = () => {
		...
		const addBooking = (modalData, recurrentId) => {
			...
			fetch(
				"http://mediabit.ro/booking-staging/wp-json/wp/v2/posts",
				requestOptions
			)
				.then((response) => response.json())
				.then((result) => {
		...
		if (!provider) {
			...
			if (!recurrent) addBooking(modalDataObj);
				...
				console.log(
					"URL",
					`http://mediabit.ro/booking-staging/wp-json/multiple/bookings/?data_start=${modalDataObj.toISOString()}&data_end=${endDate.toISOString()}&provider=${provider}&client=${
						props.id
